...
public class WorldMenu extends EditorMenu {
	...
	public WorldMenu(final LevelEditor le, String name) {
		super(le, name);
		...
		LevelEditor.addAction("Resize Level", new LevelEditor.Action() {
			@Override
		...
		final WorldMenu thisComponent = this;
		LevelEditor.addAction("Change Sky Color", new LevelEditor.Action() {
			@Override
			public void run(String[] args) {
				Color skyColor = JColorChooser.showDialog(thisComponent, "Choose Color", new Color(98, 204, 249));
				if(skyColor != null)
					le.level.skyColor = skyColor;
			}
		});
		
		LevelEditor.addAction("Test", new LevelEditor.Action() {
			@Override
			public void run(String[] args) {
				final float oldZoom = Main.zoom;
				Main.changeZoom(1);
				File file;
		...
		LevelEditor.addAction("Zoom In", new LevelEditor.Action() {
			@Override
			public void run(String[] args) {
		...
		LevelEditor.addAction("Zoom Out", new LevelEditor.Action() {
			@Override
			public void run(String[] args) {
				LevelEditorUtils.zoomOut();
			}
		});
		LevelEditor.addAction("Change Gravity", new LevelEditor.Action() {
			@Override
			public void run(String[] args) {
				String str;
				if (args.length > 1)
